...
function usage() {
  echo ""
  echo "Usage:"
  echo "    $0 [--tesseraJar path to Tessera jar file] [--remoteDebug] [--jvmParams \"JVM parameters\"] [--nodelow ] [--nodehigh ]"
  echo ""
  echo "Where:"
  ...
  echo "    --nodelow - lower node number for which tessera has to be started"
  echo "    --nodehigh - high node number for which tessera has to be started"
  echo "Notes:"
